...
public class ProductDao {
	...
	public static ProductData[] searchProducts(String idCompare, String id,
			String name, String priceCompare, String price,
			String quantityCompare, String quantity, String type,
			String supidCompare, String supid) {
		// String
		// sql2="select * from products where prod_id"+idCompare+"? and prod_name=? and price"+priceCompare+"? and type=? and quantity"+quantityCompare+"? and sup_id"+supidCompare+"?";
		String sql2 = "select * from products where 1=1";
		if (!id.equals(""))
			sql2 = sql2 + " and prod_id" + idCompare + id;
		if (!name.equals(""))
			sql2 = sql2 + " and prod_name='" + name + "'";
		if (!price.equals(""))
			sql2 = sql2 + " and price" + priceCompare + price;
		if (!quantity.equals(""))
			sql2 = sql2 + " and quantity" + quantityCompare + quantity;
		if (!type.equals(""))
		...
		try {
			...
			while (rs.next()) {
				...
				products[i] = new ProductData(_id, _name, _price, _type,
						_quantity, _supid);
				i++;
			}
			return products;
	...
	public static ProductData getProductbyID(int id) {
		...
		try {
			...
			if (rs.next()) {// 将结果封装成一个ProductData的对象并返回
				return new ProductData(id, rs.getString("prod_name"),
						Float.parseFloat(rs.getString("price")),
						rs.getString("type"), Float.parseFloat(rs
								.getString("quantity")), Integer.parseInt(rs
								.getString("sup_id")));
			} else
				return null;
		} catch (SQLException e) {
